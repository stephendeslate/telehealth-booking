...
export default function PaymentsPage() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/me/payments');
      return data;
    },
  });
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left px-6 py-3 font-medium text-gray-500">Practice</th>
            ...
            <tbody className="divide-y">
              {payments.map((p: any) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">{p.appointment?.practice?.name || '-'}</td>
